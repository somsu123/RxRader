...
export default function PrescriptionHistoryPage() {
  ...
  async function loadPrescriptions() {
    try {
      ...
      setError('Unable to load prescriptions. Please refresh the page.');
    } finally {
      setLoading(false);
    }
  }

  async function deletePrescription(id: number | undefined) {
    if (!id) return;
  ...
  function exportToPDF(prescription: Prescription) {
    try {
      const doc = new jsPDF();
      const dateStr = prescription.date instanceof Date ? prescription.date.toLocaleString() : new Date(prescription.date).toLocaleString();
  ...
  function exportAllToPDF() {
    try {
      ...
      for (let i = 0; i < prescriptions.length; i++) {
        ...
        const dateStr = p.date instanceof Date ? p.date.toLocaleString() : new Date(p.date).toLocaleString();
        ...
        doc.setFontSize(10);
        doc.setTextColor(100, 100, 100);
        doc.text(`Date: ${dateStr}`, 20, yOffset + 7);

        const medicines = p.medicines || [];
        const tableData = medicines.map(med => [med.name, med.quantity.toString(), `₹${med.price}`, `₹${med.price * med.quantity}`]);
  ...
  const filteredPrescriptions = safePrescriptions.filter(p => {
    ...
    const now = new Date();
    const presDate = p.date instanceof Date ? p.date : new Date(p.date);
  ...
  const totalSavings = filteredPrescriptions.reduce((sum, p) => sum + (p.savingsAmount || 0), 0);
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          {error && (
            <div className="bg-red-50 border border-red-200 rounded-lg p-4 mb-6">
              ...
              <button onClick={loadPrescriptions} className="mt-2 text-sm text-red-700 underline">Retry</button>
            </div>
          )}
