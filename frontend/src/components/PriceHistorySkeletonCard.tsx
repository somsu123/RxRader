export default function PriceHistorySkeletonCard() {
  return (
    <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 p-6 flex flex-col animate-pulse">
      ...
      <div className="h-32 flex items-end justify-between gap-1.5 mb-2 mt-auto">
        {Array.from({ length: 12 }).map((_, i) => (
          <div key={i} className="flex-1 flex flex-col items-center gap-1.5 h-full justify-end">
            ...
            />
            <div className="h-2 w-3 bg-slate-100 dark:bg-slate-600 rounded" />
          </div>
        ))}
      </div>
